		...
		if (exec !== null) {
			...
			$.ajax({
				...
				if (decomposedURI.uriHost === "www.tiktok.com") {
					...
					console.log("3587 uriRendered: ", uriRendered);
					uriRendered.newURI = resp;
					resolve({ ...uriRendered, ...decomposedURI });
				}
				else {
					resolve({ html: (toA ? `<a target="_blank" href="${shortURI}">${shortURI}</a>` : ""), from: "-tiktok-link", ...decomposedURI });
				}
			});
		}
	});
...
ptnURI.toIframe = function (uri, inListPlay) {
	return new Promise(function (resolve, reject) {
		...
		if (exec !== null) {
			return resolve({ html: `<a target="_blank" href="https://kr75.sogirl.so${exec[1] ? exec[1] : ""}">${m.escapeOnlyTag(decodeURIComponent(`https://kr75.sogirl.so${exec[1] ? exec[1] : ""}`))}</a>`, newURI: `https://kr75.sogirl.so${exec[1] ? exec[1] : ""}`, from: 'sogirl', src: exec[1] });
		}
		else {
...
ptnURI.toIframe = function (uri, inListPlay) {
	return new Promise(function (resolve, reject) {
		...
		if (exec !== null) {
			resolve({ html: `<a target="_blank" href="https://kr37.topgirl.co${exec[1] ? exec[1] : ""}">${m.escapeOnlyTag(decodeURIComponent(`https://kr37.topgirl.co${exec[1] ? exec[1] : ""}`))}</a>`, newURI: `https://kr37.topgirl.co${exec[1] ? exec[1] : ""}`, from: 'topgirl', src: exec[1] });
		}
		else {
			reject(false);
		}
...
m.decomposeURI = function (uri) {
	uri = String(uri);
	uri = uri.split(/[\s\t\n\r]+/)[0];
	const res = { uri, protocol: null, uriHost: null, pathname: null, search: null, hash: null }; // uriHost, pathname, search, hash
...
m.uriRendering = function (uri, toA, inListPlay, descR) {
	return new Promise(async function (resolve, reject) {
		...
		if (uriHost) {
			...
			if (toA) {
				result = { html: m.uriToA(uri) };
				return resolve({ ...result, ...decomposedURI, uri });
			}
		}
		result = { html: m.escapeOnlyTag(decodeURIComponent(uri)) };
		return resolve({ ...result, ...decomposedURI, uri });
	});
};
...
m.docuKProcess = function docuK(m, $, docuKI, undefined) {
	...
	docuKI = (isNaN(docuKI) || docuKI < 1) ? 1 : parseInt(docuKI);
	...
	if ($docuKI.is(".noDIdHandle")) {
		postId = "";
	}
	else {
		$docuKI.find("[id]").each(function () {
			let $this = $(this);
			$this[0].id += postId;
		});
	}
